...
TLBhierarchy::TLBhierarchy(ComponentId_t id, int tlb_id, int Levels, Params& params) : ComponentExtension(id)
{
	...
	// Add the a list to track memory requests from PTW
	memory_requests_from_ptw = PTW->get_mem_requests();
...
void TLBhierarchy::handleEvent_CACHE( SST::Event * ev )
{
	...
	// Here we need to distinguish between requests coming for translations vs. regular data requests
	
	MemEvent* mEvent = static_cast<MemEvent*>(ev);

	long long int addr = mEvent->getVirtualAddress();

	bool regular_request=true;
	if(ptw_addresses.find(addr)!=ptw_addresses.end())
	...
	ptw_addresses[addr]--;
	if(ptw_addresses[addr]==0)
		ptw_addresses.erase(addr);
	PTW->push_response(ev);
...
bool TLBhierarchy::tick(SST::Cycle_t x)
{
	...
	// Check if there is any requests from PTW to memory
	if(!(*memory_requests_from_ptw).empty())
	{

//	printf("Processing PTW REQUEST \n");	
	MemEvent * ptw_event = (*memory_requests_from_ptw).back();
	(*memory_requests_from_ptw).pop_back();	

	ptw_addresses[ptw_event->getVirtualAddress()]++;

	to_cache->send(ptw_event);
