...
namespace SST { namespace SambaComponent{
	...
	class PageTableWalker : public ComponentExtension
	{
		...
		std::vector<SST::Event*> cache_responses;

		std::vector<SST::MemHierarchy::MemEvent *> * mem_requests;
		...
		std::vector<SST::MemHierarchy::MemEvent *> * get_mem_requests()
		{
		  return mem_requests;
		...
		void push_response(SST::Event * ev) {cache_responses.push_back(ev);}
