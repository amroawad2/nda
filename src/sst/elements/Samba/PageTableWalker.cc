...
PageTableWalker::PageTableWalker(ComponentId_t id, int tlb_id, PageTableWalker * Next_level, int level, SST::Params& params) : ComponentExtension(id)
{
	...
	mem_requests = new std::vector<MemEvent *>();
...
void PageTableWalker::recvOpal(SST::Event * event)
{
	...
	std::cout<<"Opal Event is coming..."<<std::endl;
	...
	std::cout<<"End of Opal Event"<<std::endl;
...
void PageTableWalker::recvResp(SST::Event * event)
{
	...
	else
	{
		...
		Address_t dummy_add = rand()%1000000000;
		...
		// here we set the virtual address of page table walks similar to physical address -- page table walk accesses typically happen using physical address, but since it is coming from the cache port, we need the virtual address set
		e->setVirtualAddress(dummy_base_add);

		WSR_COUNT[pw_id]--;
		MEM_REQ[e->getID()]=pw_id;
		//to_mem->send(e);

	//	printf("PUSHING REQUEST to mem_requests \n");
		(*mem_requests).push_back(e);
...
bool PageTableWalker::tick(SST::Cycle_t x)
{
	...
	// This extra code here basically manually calls the handler for cache responses for page table walk operations received by TLB Hierarchy
	if(!cache_responses.empty())
	{

//	printf("MANUALLY CALLING recvResp \n");
	SST:Event * resp_ev = cache_responses.back();
	cache_responses.pop_back();
	recvResp(resp_ev);
